...
class TimeMatrixGenerator(data: Movement.Time) : MatrixGenerator<Movement.Time>(data) {
    ...
    private fun getTimeMatrix(time: Movement.Time): List<List<ClockData>> {
        ...
        val localTime = time.date.toLocalDateTime(TimeZone.currentSystemDefault())
        val timeSplit = listOf(
            localTime.hour / 10, localTime.hour % 10,
            localTime.minute / 10, localTime.minute % 10,
            localTime.second / 10, localTime.second % 10,
        )
        ...
        val seconds = (timeSplit[4] * 10 + timeSplit[5]).coerceIn(0, 60)
                ...
                .map {
                    it.toMutableList().map { clockItem ->
                        clockItem.copy(
                            ...
                            timeSign = time.date.toEpochMilliseconds()
                        )
                    }.toMutableList()
                }
                .toMutableList()
